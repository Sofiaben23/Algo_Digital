...
1. Mettre tous les mots en minuscule
2. Enlever toutes la ponctuations
3. Faire des n-gram (1,2 et 3)
4. Faire une intersection entre les mots/listes de mots les plus utilisés selon les types
5. Faire des clusters: quelles expressions sont les plus utilisées selon les types
- ex: mots utilisés chez les couples - intersection avec les autres types de voyageurs
...
##Tokenisation 1-gram sans les stopwords
*Contenu négatif*
```{r}
library(tidytext)
library(tm)
...
On remarque qu'il est plus pertinent de travailler avec des 3-gram pour les contenu des colonnes de commentaires.

```{r}
#créer des trigrammes à partir des colonnes de texte
comm_booking_trigrams <- comm_booking %>%
  unnest_tokens(output = trigram, input = Contenu.positif, token = "ngrams", n = 3) %>%
  bind_rows(comm_booking %>%
...
trigram_freq_by_type <- comm_booking_trigrams %>%
...
distinctive_trigrams <- trigram_freq_by_type %>%
  group_by(trigram) %>%
  filter(n() > 1) %>%
...
##Adjective noun pairs (prend bcp de temps)
...
```{r}
# Télécharger un modèle de langue française (ou une autre langue appropriée)
model <- udpipe_download_model(language = "french")

# Charger le modèle
ud_model <- udpipe_load_model(model$file_model)

# Préparer vos données (exemple ici avec 'Contenu.positif')
text_data <- data.frame(text = as.character(comm_booking$Contenu.positif), stringsAsFactors = FALSE)
```

```{r}
# Appliquer le POS tagging
tagged_text <- udpipe_annotate(ud_model, x = text_data$text)
tagged_df <- as.data.frame(tagged_text)

```

```{r}
# Filtrer les adjectifs et les noms, et les lier
adj_noun_pairs <- tagged_df %>%
  filter(upos %in% c("ADJ", "NOUN")) %>%
  group_by(doc_id, paragraph_id, sentence_id) %>%
  arrange(doc_id, paragraph_id, sentence_id, token_id) %>%
  mutate(next_word = lead(lemma), next_pos = lead(upos)) %>%
  filter(upos == "ADJ" & next_pos == "NOUN") %>%
  select(adj = lemma, noun = next_word)
