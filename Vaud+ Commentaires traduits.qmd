...
ggplot(data_pourcentage, aes(x = Pays, y = Pourcentage, fill = Type))+
...
data_pourcentage_plot <- data_pourcentage %>% 
  ...
  geom_bar(stat = "identity") +
  theme_minimal()
data_pourcentage_plot

ggplotly(data_pourcentage_plot)

tmp_data <- tibble(xax = c(1, 2, 3, 4), yax = c(2, -1, 0, 1))
tmp_data_plot <- tmp_data %>% 
  ggplot(aes(x = xax, y = yax)) + 
  geom_line()
tmp_data_plot

tmp_data_plot %>% ggplotly()
...
# Tokenisation de la colonne Contenu.positif
tokens_positif <- subset_comments %>%
  unnest_tokens(word, Contenu.positif, token = "words")

# Vous pouvez maintenant manipuler les tokens ou les combiner selon vos besoins
# Par exemple, pour les combiner en un seul dataframe :
tokens_combines <- bind_rows(tokens_negatif, tokens_positif)
view(tokens_combines)
````
...
##N-gram (2 ou 3 gram)
##Tag Cloud (passer le texte en minuscule)
